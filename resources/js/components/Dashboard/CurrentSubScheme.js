...
import {getCurrentSubSchemesReport,getCurrentSubSchemesHeaderReport} from '../store/subschemes-actions';
...
function CurrentSubScheme() {
  ...
  useEffect(() => {
    dispatch(getCurrentSubSchemesReport(params.id));
    dispatch(getCurrentSubSchemesHeaderReport(params.id));
    return () => {
  ...
  return (
    ...
    <table className="f6 w-100 center mb3 ba" cellSpacing="0">
      <tr style={{"borderWidth":"1px", 'borderColor':"#000000", 'borderStyle':'solid', textAlign:"center"}}>
        <td colSpan='12' style={{"borderWidth":"1px", 'borderColor':"#000000", 'borderStyle':'solid', textAlign:"center", fontWeight: 'bold'}}>{`Detailed Report`}</td>
      </tr>
      ...
      <tbody className="lh-copy ba">
        {(toDisplay === 'outputindicators') && (report.status === 200) && report.outputindicators.map((indicator,i) => {
          ...
          return (
            ...
            {(i === 0) ? <td className="pv3 pr3 reportextcolor b--black-80" rowSpan={rowofspan} style={{"borderWidth":"1px", 'borderColor':"#000000", 'borderStyle':'solid', 'textAlign':"center", 'verticalAlign': "middle"}}>{report.subscheme.initial_remarks ? report.subscheme.initial_remarks.split('\n').map(function(item) {
  return (
    <span>
      {item}
      <br/>
...
export default CurrentSubScheme