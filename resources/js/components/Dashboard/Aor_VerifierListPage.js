...
import {aorgetSubschemeData,aorgetComment,aorapproveDivisionSubschemes,sendApprovalMail} from '../store/departmentVerifier-actions';
...
const AorVerifierListPage = () => {
  ...
  const onSubmitApprove = () => {
    ...
    const f = subschemes.map(subscheme => {
      if((subscheme.aor_verified_status === "N") || (subscheme.aor_verified_status === "R") || (subscheme.aor_verified_status === "T")){
        return 'a'
      }
      return 'b'
    })
    ...
    dispatch(aorapproveDivisionSubschemes(divisionid));
  ...
  const handleActionClick = (id) => {
    ...
    return navigate(`/dashboardverifier/aordetailreport/${id}`);
}
...
useEffect(() => {
  ...
  >
    ...
    <h1 className='b'>Dashboard for verification of sub-schemes for On Demand Report</h1>
        <div className="row">
...
export default AorVerifierListPage