...
import {storeOutputData} from '../store/output-actions';
...
function CreateOutput() {
  ...
  const outputmessage = useSelector((state) => state.outputs.message);
  ...
  const [subSchemeSearchResult, setSubSchemeSearchResult] = useState({})
    ...
    useEffect(() => {
      ...
      return () => {
        ...
        dispatch(outputsActions.setMessage({message:''}))
  ...
  const onSubmitOutput = () => {
    if(divisionId === '') {
      return dispatch(outputsActions.setMessage({message:'No Division Selected'}))}
    
    if(Object.keys(schemeSearchResult).length === 0) {
      return dispatch(outputsActions.setMessage({message:'No Scheme Selected'}))}

      if(Object.keys(subSchemeSearchResult).length === 0) {
        return dispatch(outputsActions.setMessage({message:'No Sub Scheme Selected'}))}
    
    if(outputname === '') {
      return dispatch(outputsActions.setMessage({message:'Please Enter Output Name'}))}
    
    dispatch(storeOutputData(departmentId,divisionId,schemeSearchResult.id,subSchemeSearchResult.id,outputname))
    setDepartmentId('')
    setDivisionId('')
    setSchemeSearchType('statename')
    setSchemeSearchResult({})
  ...
  return (
      ...
      <h1 className='b'>Create Output for Sub Scheme</h1>
...
<div className="pa4 black-80">
    ...
    <small id="name-desc" className="f6 black-60 db mb2">Helper text for the form control.</small>
    {(outputmessage !== '') && <p className="f6 ph3 pv2 mb2 mt2 black w-100 tc pointer b ba b--black-40">{outputmessage}</p>}
  <p className="f6 link dim br3 ph3 pv2 mb2 dib mt2 white bg-dark-green w5 tc pointer shadow-3" onClick={onSubmitOutput}>Create Output</p>
</div>
      </div>
  )
}
