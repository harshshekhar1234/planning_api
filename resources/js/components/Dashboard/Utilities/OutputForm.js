...
export default function OutputForm() {
  ...
  const handleNewOutputSubmit = () => {
    ...
    const output = {
        id:newOutputId,
        name:outputname,
        outputIndicators:outputIndicatorRows
      };

    const newOutput = [...outputs,output]

    setNewOutputId(prevCount => prevCount + 1)
    dispatch(migrationActions.setOutputs({outputs:newOutput}))
    handleClose1()
  ...
  const [open2, setOpen2] = React.useState(false);
    ...
    const handleOutputIndicatorEditSubmit = () => {
      if(editOutputIndicator.name === ''){
        setOutputEditMessage('Please enter Output Indicator')
        return
      }
      if(!isNaN(editOutputIndicator.name)){
        setOutputEditMessage('Please enter valid Output Indicator')
        return
      }
      if(editOutputIndicator.targetValue === ''){
        setOutputEditMessage('Please enter Output Indicator Target Value')
        return
      }
      if(isNaN(editOutputIndicator.targetValue)){
        setOutputEditMessage('Please enter numeric value for Output Indicator Target Value')
        return
      }
      if(editOutputIndicator.targetMeasurement === ''){
        setOutputEditMessage('Please enter Output Indicator Target Measurement Unit')
        return
      }
      ...
      if(editOutputIndicator.targetYear === ''){
        setOutputEditMessage('Please enter Output Indicator Target Year')
        return
      }
      if(isNaN(editOutputIndicator.targetYear)){
        setOutputEditMessage('Please enter a valid Output Indicator Target Year')
        return
      }
      if(editOutputIndicator.targetYear < 2022){
        setOutputEditMessage('Please enter a valid Output Indicator Target Year')
        return
      }
      if(editOutputIndicator.targetYear > 2023){
        setOutputEditMessage('Please enter a valid Output Indicator Target Year')
        return
      }

      
      const newOutput = outputs.filter(output => output.id === editOutputId)
      const b = newOutput[0].outputIndicators.filter(indicator => indicator.id !== editOutputIndicator.id)
      const c = [...b,editOutputIndicator]
      const d = {...newOutput[0],outputIndicators:c}
      const editOutput = outputs.filter(output => output.id !== editOutputId)
      const h = [...editOutput,d]
      dispatch(migrationActions.setOutputs({outputs:h}))
      handleClose8()
    ...
    const handleNewOutputIndicatorSubmit = () => {
      ...
      const newOutput = outputs.filter(output => output.id === editOutputId)
      const c = [...newOutput[0].outputIndicators,...outputIndicatorRows]
      const d = {...newOutput[0],outputIndicators:c}
      const editOutput = outputs.filter(output => output.id !== editOutputId)
      const h = [...editOutput,d]
      dispatch(migrationActions.setOutputs({outputs:h}))
    ...
    const handleOpen15 = (id) => {
      ...
      handleClose2()
      const newOutput = outputs.filter(output => output.id === editOutputId)
      const count =  newOutput[0].outputIndicators.length
      if(count === 1){
    ...
    const handleOutputIndicatorDelete = () => {
      const newOutput = outputs.filter(output => output.id === editOutputId)
      const b = newOutput[0].outputIndicators.filter(indicator => indicator.id !== indicatorDeleteId)
      const d = {...newOutput[0],outputIndicators:b}
      const editOutput = outputs.filter(output => output.id !== editOutputId)
      const h = [...editOutput,d]
      dispatch(migrationActions.setOutputs({outputs:h}))
      handleClose15()
    ...
    const handleUpdateOutput = () => {
      if(editOutputName === ''){
        setUpdateOutputMessage('Please enter output')
        return
      }
      const newOutput = outputs.filter(output => output.id === updateOutputId)
      const d = {...newOutput[0],name:editOutputName}
      const editOutput = outputs.filter(output => output.id !== updateOutputId)
      const h = [...editOutput,d]
      dispatch(migrationActions.setOutputs({outputs:h}))
      handleClose13()
    }
  ...
  <div className="overflow-auto">
    ...
    <small id="name-desc" className="f6 black-60 db mb2">Helper text for the form control.</small>
      ...
      <Button size="large" onClick={handleNewOutput}>+ Create New Output</Button>
      <Box
    ...
    >
      ...
      >
        ...
        return <Tab label={`Output ${i + 1}`} {...a11yProps(i)} />
        })}
      </Tabs>
      {outputs && outputs.map((output,i) => {
    ...
    >
      <ButtonGroup variant="text" size="small" aria-label="text button group">
        <Button onClick={()=> handleOpen13(output.id,output.name)}>Edit Output</Button>
        <Button onClick={()=> handleOpen17(output.id)}>Delete Output</Button>
        <Button onClick={()=> handleOpen2(output.outputIndicators,output.id)}>View Indicator</Button>
