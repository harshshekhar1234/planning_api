...
import {getQuaterReportList} from '../store/quaterReport-actions';
...
function QuaterWiseReportVerifier() {
  ...
  useEffect(() => {
      dispatch(getQuaterReportList(divisionid))
      return () => {
  ...
  const handleActionClick = (params) => {
      dispatch(quaterReportActions.setQuater({quater:params.row.name}))
      dispatch(quaterReportActions.setFinancialYear({financialyear:params.row.year}))
      return navigate(`/dashboardverifier/quaterreportidverifier/${divisionid}/${params.id}`);
  }
  ...
  const columns = React.useMemo(
    () => [
      ...
      { field: 'name', hideable: false, headerName: 'Quater', headerAlign: 'center',headerClassName: 'themeheader',cellClassName: 'themecell', flex: 1,
      renderCell: handleQuaterDisplay
    },
...
export default QuaterWiseReportVerifier
