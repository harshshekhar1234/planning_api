...
import {storeOutcomeIndicatorData} from '../store/outcomeindicator-actions';
...
function CreateOutcomeIndicator() {
  ...
  const outcomeindicatorname = useSelector((state) => state.outcomeindicator.outcomeindicatorName);
  const outcomeindicatormessage = useSelector((state) => state.outcomeindicator.message);
  ...
  const [subSchemeSearchResult, setSubSchemeSearchResult] = useState({})
    ...
    useEffect(() => {
      ...
      return () => {
        ...
        dispatch(outcomeindicatorActions.setMessage({message:''}))
      }
    }, []);
  ...
  const onSubmitOutcomeIndicator = () => {
    if(divisionId === '') {
      return dispatch(outcomeindicatorActions.setMessage({message:'No Division Selected'}))}
    
    if(Object.keys(schemeSearchResult).length === 0) {
      return dispatch(outcomeindicatorActions.setMessage({message:'No Scheme Selected'}))}

      if(Object.keys(subSchemeSearchResult).length === 0) {
        return dispatch(outcomeindicatorActions.setMessage({message:'No Sub Scheme Selected'}))}

     if(outcomeId === '') {
      return dispatch(outcomeindicatorActions.setMessage({message:'No Outcome Selected'}))}
    
    if(outcomeindicatorname === '') {
      return dispatch(outcomeindicatorActions.setMessage({message:'Please Enter Outcome Indicator'}))}

    dispatch(storeOutcomeIndicatorData(departmentId,divisionId,schemeSearchResult.id,subSchemeSearchResult.id,outcomeId,outcomeindicatorname))
  }

  const handleOutcomeIndicatorNameChange = (event) => {
    dispatch(outcomeindicatorActions.setMessage({message:''}))
    dispatch(outcomeindicatorActions.setOutcomeIndicatorName({outcomeindicatorName:event.target.value}))
  }
  ...
  return (
      ...
      <h1 className='b'>Create Outcome Indicator</h1>
...
<div className="pa4 black-80">
  <div className="measure">
    ...
    <select id="department_name" className="ba b--black-60 pa2 mb2 db w-100 bg-transparent br2 shadow-1" type="text" aria-describedby="name-desc" defaultValue={'default'} onChange={handleOutcomeChange}>
  ...
  {(outcomesearchmessage !== "") && <div className="flex items-center justify-center pa2 ml4 bg-light-red w-60 navy br2 shadow-2">
  <svg className="w1" data-icon="info" viewBox="0 0 32 32" style={{"fill":"currentcolor"}}>
    <title>info icon</title>
    <path d="M16 0 A16 16 0 0 1 16 32 A16 16 0 0 1 16 0 M19 15 L13 15 L13 26 L19 26 z M16 6 A3 3 0 0 0 16 12 A3 3 0 0 0 16 6"></path>
  </svg>
  <span className="lh-title ml3 black">{outcomesearchmessage}</span>
</div>}
</div>
<div className="pa4 black-80">
    <label htmlFor="name" className="f6 b db mb2">Please Enter Outcome Indicator</label>
    <input id="name" className="ba b--black-60 pa2 mb2 db w-100 bg-transparent br2 shadow-1" type="text" aria-describedby="name-desc" onChange={handleOutcomeIndicatorNameChange} value={outcomeindicatorname}/>
    <small id="name-desc" className="f6 black-60 db mb2">Helper text for the form control.</small>
    {(outcomeindicatormessage !== '') && <p className="f6 ph3 pv2 mb2 mt2 black w-100 tc pointer b ba b--black-40">{outcomeindicatormessage}</p>}
  <p className="f6 link dim br3 ph3 pv2 mb2 dib mt2 white bg-dark-green w5 tc pointer shadow-3" onClick={onSubmitOutcomeIndicator}>Create Outcome Indicator</p>
</div>
      </div>
  )
}

export default CreateOutcomeIndicator