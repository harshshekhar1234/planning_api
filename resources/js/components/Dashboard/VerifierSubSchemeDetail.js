...
import {getSubSchemesReportVerifier,getSubSchemesHeaderReportVerifier} from '../store/subschemes-actions';
import {approveSubscheme,reviewSubscheme,getLatestQuater} from '../store/departmentVerifier-actions';
...
function VerifierSubSchemeDetail() {
  ...
  useEffect(() => {
    dispatch(getSubSchemesReportVerifier(params.id));
    dispatch(getSubSchemesHeaderReportVerifier(params.id));
    ...
    const handleApprove = () => {
        dispatch(approveSubscheme(params.id));
    }
    ...
    const onSubmitComments = () => {
      ...
      dispatch(reviewSubscheme(params.id,comment));
      handleClose1()
    }
  ...
  return (
    ...
    <table className="f6 w-100 center mb3 ba" cellSpacing="0">
      <tr style={{"borderWidth":"1px", 'borderColor':"#000000", 'borderStyle':'solid', textAlign:"center"}}>
        <td colSpan='15' style={{"borderWidth":"1px", 'borderColor':"#000000", 'borderStyle':'solid', textAlign:"center", fontWeight: 'bold'}}>{`Detailed Report ( For ${quaterTD} FY- ${yearTD})`}</td>
      </tr>
      ...
      <tbody className="lh-copy ba">
        {(toDisplay === 'outputindicators') && (report.status === 200) && report.outputindicators.map((indicator,i) => {
          ...
          return (
            ...
            {(i === 0) ? <td className="pv3 pr3 reportextcolor b--black-80" rowSpan={rowofspan} style={{"borderWidth":"1px", 'borderColor':"#000000", 'borderStyle':'solid', 'textAlign':"center", 'verticalAlign': "middle"}}>{report.subscheme.risk_remarks ? report.subscheme.risk_remarks.split('\n').map(function(item) {
  return (
    <span>
      {item}
      <br/>
...
export default VerifierSubSchemeDetail