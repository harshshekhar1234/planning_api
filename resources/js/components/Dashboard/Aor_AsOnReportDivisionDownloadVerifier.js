...
import {aorgetSubSchemesDivisionReportId} from '../store/quaterReport-actions';
...
import SubSchemeReportDivisionAsOn from './Aor_SubschemeReportDivisionAsOn';
...
function AsOnReportDivisionDownloadVerifier() {
  ...
  const fromDate = useSelector((state) => state.quaterreport.fromDate);
  const toDate = useSelector((state) => state.quaterreport.toDate);

  useEffect(() => {
    dispatch(aorgetSubSchemesDivisionReportId(params.id,params.rid));
    return () => {
      dispatch(quaterReportActions.setDivisionReportId({divisionReportId: {}})) 
      dispatch(quaterReportActions.setFromDate({fromDate: ''}))
      dispatch(quaterReportActions.setToDate({toDate: ''}))
    }
    }, []);
    ...
    )}
    {
      (divisionreport.status === 200) ? (
        divisionreport.subschemes.map(report => {
          return <SubSchemeReportDivisionAsOn report={report} fromDate={fromDate} toDate={toDate}/>
        })
      ):(
        <div>
        <Box sx={{ width: '100%' }}>
...
export default AsOnReportDivisionDownloadVerifier