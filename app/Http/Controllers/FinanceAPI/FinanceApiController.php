...
class FinanceApiController extends Controller
{
    public function pending_scheme($id)
    {
        ...
        $map = collect($api_schemes)->map(function ($items) use ($data) {
            ...
            $data['state_share'] = $items->S_DEPT_BE / 100000;
            $data['center_share'] = $items->C_DEPT_BE / 100000;
    ...
    public function subscheme_outlay($id)
    {
        ...
        ->where('ss.id', $id)->get();
        if($sub_scheme->isEmpty())
        {
            return response()->json([
                'status' => 404,
                'error' => "Sub-Scheme not Found in API",
            ]);
        }

        $demand_no = $sub_scheme[0]->demand_no;
        $demand_no = sprintf("%02d", $demand_no);
        ...
        $response = Http::acceptJson()->get('http://jkuber.jharkhand.gov.in/outcomebudgetservice/OutcomeScheme.svc/getOutcomeBudgetOutlay?demand=' . $demand_no . '&finyear='.$this->api_fin_year.'&statecode=' . $sub_scheme[0]->state_code . '&central=' . $sub_scheme[0]->center_code . '&pwd='.$this->api_password);
        $api_subscheme = json_decode($response);
        $api_subscheme = json_decode($api_subscheme->getOutcomeBudgetOutlayResult);
        ...
        $subscheme = collect($api_subscheme)->filter(function ($value, $skey) use ($sub_scheme) {
            return ($value->SUB_SCHEMECODE == $sub_scheme[0]->subscheme_code);
        });

        if($subscheme->isEmpty())
        {
            return response()->json([
                'status' => 404,
                'error' => "Sub-Scheme not Found in API",
        ...
        $subscheme = $subscheme->values()->all();
        $SubschemeOutlay = ($subscheme[0]->S_DEPT_BE + $subscheme[0]->C_DEPT_BE) / 100000;
        return response()->json([
            'status' => 200,
            'result' => $SubschemeOutlay,
            'state_share' => $subscheme[0]->S_DEPT_BE / 100000,
            'center_share' => $subscheme[0]->C_DEPT_BE / 100000
        ]);
    }
    ...
    public function subscheme_header($id)
    {
        ...
        $subscheme = $subscheme->values()->all();
        $sub_scheme[0]->name = $subscheme[0]->SUB_SCHEMEENAME;
        $sub_scheme[0]->state_name = ($subscheme[0]->STATESCHEMENAME == null) ? $subscheme[0]->STATESCHEMECODE : $subscheme[0]->STATESCHEMENAME;
        $sub_scheme[0]->state_code = $subscheme[0]->STATESCHEMECODE;
        $sub_scheme[0]->center_name = ($subscheme[0]->GOISCHEMENAME == null) ? $subscheme[0]->CPSMSSCHEME_CODE : $subscheme[0]->GOISCHEMENAME;
        $sub_scheme[0]->center_code = $subscheme[0]->CPSMSSCHEME_CODE;
        return response()->json([
            'status' => 200,
